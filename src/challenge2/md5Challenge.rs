...
pub struct md5Chall{input:StructInput}
...
impl md5Chall{
    pub fn start(&self) -> StructOutput {
        ...
        while finish == false {
            ...
            concat_seed = String::from(Self::concat_string(hex_seed.to_string(), self.input.message.to_string()).trim());
...
let chall:md5Chall=md5Chall { input: (StructInput { complexity: (1), message: ("2".to_owned()) }) };
}                      
